...
| 5 | Auth | **Two layers, different reach (REVISED).** (a) Static API key(s) via `Authorization: Bearer <key>` (also `X-API-Key`, `?api_key=`); constant-time compare; multiple keys for per-caller revocation. Enforced as a per-route **dependency**, so it covers `/api/*` only. (b) **HTTP Basic** (`APPSECWATCH_BASIC_AUTH=user:password`) as ASGI middleware on the **parent** app, covering **every** path including the static UI mount. Either credential is sufficient; `/healthz` is exempt. | (a) is simple and sufficient service-to-service, but a per-route dependency structurally cannot guard the statically-mounted SPA — which left the UI shell anonymous. (b) closes that with a browser-native prompt and no identity provider. Both authorise the same action (driving scans), so accepting either adds no privilege. |
