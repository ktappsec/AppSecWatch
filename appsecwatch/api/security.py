...
def sign_webhook(body: bytes, secret: str) -> str:
    """`sha256=<hex>` HMAC-SHA256 of the raw body (matches the X-AppSecWatch-Signature)."""
...
async def send_webhook(
    ...
    headers = {
        ...
        "X-AppSecWatch-Event": event,
    }
    if server.webhook_secret:
        headers["X-AppSecWatch-Signature"] = sign_webhook(body, server.webhook_secret)
