...
from appsecwatch.example_config import EXAMPLE_CONFIG_YAML
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-V", "--version", action="version", version=f"appsecwatch {__version__}")
    ...
    serve.add_argument("--ui-dir", default=None,
                       help="Serve a built UI (Next static export) from this dir at '/' "
                            "with the API under '/api'. Defaults to $APPSECWATCH_UI_DIR.")
...
def _cmd_scan(args: argparse.Namespace) -> int:
    from appsecwatch.config import load_config
    from appsecwatch.runner import run_scan
    from appsecwatch.stages.pipeline import SelectionError, resolve_selection
...
def _cmd_serve(args: argparse.Namespace) -> int:
    ...
    try:
        from appsecwatch.api.server import serve
    except ImportError as e:
        print(
            f"serve requires the web extras (fastapi, uvicorn): {e}\n"
            "Install with: pip install 'appsecwatch[web]'  (or pip install fastapi 'uvicorn[standard]')",
            file=sys.stderr,
        )
        return 1
    try:
...
def _cmd_verify_deps(args: argparse.Namespace) -> int:
    from appsecwatch.preflight import format_report, run_preflight
