...
class Capability:
    ...
    """
    factory: Callable[[AppSecWatchConfig, "SelectionPlan"], Stage | None]
    phase: str
...
def _takeovers(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage:
    from appsecwatch.stages.audit import TakeoversStage
    return TakeoversStage()


def _tls(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage:
    from appsecwatch.stages.audit import SslscanStage
    return SslscanStage()


def _nuclei(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage:
...
def _crawler(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage:
    from appsecwatch.stages.audit import CrawlerStage
    return CrawlerStage()


def _headers(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage | None:
    from appsecwatch.stages.audit import HeadersStage
...
def _ai(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage | None:
    from appsecwatch.stages.ai import AIStage
...
def _zap(cfg: AppSecWatchConfig, plan: SelectionPlan) -> Stage | None:
