...
The actual stage logic lives in appsecwatch.stages.*. This module wires bootstrap
...
from appsecwatch.stages.report_stage import CompressStage, ReportStage
from appsecwatch.stages.state import ScanState
from appsecwatch.util.ipinfo import IPInfoLookup
from appsecwatch.util.subproc import tool_version
...
async def collect_versions(cfg: AppSecWatchConfig) -> dict[str, Any]:
    binaries = ["subfinder", "dnsx", "tlsx", "httpx", "nuclei"]
    versions: dict[str, Any] = {"appsecwatch": __version__}
...
def _snapshot_config(cfg: AppSecWatchConfig, run_dir: Path) -> None:
...
def _log_throttle(cfg: AppSecWatchConfig, log: RunLogger) -> None:
...
async def _run(
    ...
    *,
    ...
    run_meta = {
        ...
        "duration": "",
        "appsecwatch_version": __version__,
    }
