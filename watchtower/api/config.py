...
env-resolved secrets are the API's own auth (`WATCHTOWER_API_KEYS`) + the webhook
signing secret; the LLM api_key is UI-managed and persists in the store
(`WATCHTOWER_LLM_API_KEY` only seeds first boot). There is **no scan-target
...
validated `WatchTowerConfig`, because per-request overrides (roots, throttle) must
...
ENV_API_KEYS = "WATCHTOWER_API_KEYS"
ENV_WEBHOOK_SECRET = "WATCHTOWER_WEBHOOK_SECRET"
ENV_LLM_API_KEY = "WATCHTOWER_LLM_API_KEY"
# Path to the writable runtime config store (overrides the default under output_root).
ENV_CONFIG_STORE = "WATCHTOWER_CONFIG_STORE"
...
class ConfigManager:
    ...
    def update(self, base_config: dict[str, Any]) -> dict[str, Any]:
        ...
        WatchTowerConfig.model_validate(probe)  # raises ValidationError on bad input
    ...
    def set_prompt_overrides(self, prompts: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        from watchtower.config import AIPromptsConfig
