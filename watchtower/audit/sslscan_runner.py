...
from watchtower.config import SslscanConfig
...
def build_sslscan_cmd(host: str, port: int, xml_path: Path, cfg: SslscanConfig) -> list[str]:
    ...
    `--no-failed` trims the XML to negotiated ciphers only. Options must precede
    the `host:port` target, so any `extra_flags` go before it.
    """
...
def _check_protocols(ssltest: ET.Element) -> list[TLSCheck]:
    ...
    for ptype, pver, label in (("ssl", "2", "SSL 2.0"), ("ssl", "3", "SSL 3.0"),
        ...
        checks.append(TLSCheck(
            name=f"{label} disabled", passed=not on,
...
def _check_certificate(ssltest: ET.Element) -> list[TLSCheck]:
    ...
    if cert is None:
        return [TLSCheck(name="Certificate scan", passed=False,
    ...
    if not_after is None:
        checks.append(TLSCheck(name="Cert valid and >30 days remaining", passed=False,
                               detail="could not parse not-valid-after", severity="high"))
    else:
        days = (not_after - datetime.now(timezone.utc)).days
        checks.append(TLSCheck(name="Cert valid and >30 days remaining", passed=days > 30,
                               detail=f"{days} days remaining", severity="high"))
    ...
    if pk is not None:
        ...
        ok = kbits >= floor
        checks.append(TLSCheck(name="Strong key (RSA≥2048 / EC≥256)", passed=ok,
    ...
    if sig:
        ...
        checks.append(TLSCheck(name="Strong signature algorithm", passed=not weak_sig,
                               detail=sig, severity="medium"))
    return checks
