...
export function FindingsTable({
  ...
  const groups = React.useMemo(() => {
    ...
    const m = new Map<string, { rep: Finding; hosts: string[]; count: number }>();
    for (const f of filtered) {
      const k = `${f.source}|${findingKey(f)}`;
      const host = f.host ?? "";
      const g = m.get(k);
      if (!g) m.set(k, { rep: f, hosts: host ? [host] : [], count: 1 });
      else {
        g.count++;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        ...
        finding{visible.length === 1 ? "" : "s"} (collapsed by host).
      </p>
              ...
              {onSuppress && (
                <TableCell className="text-right">
                  <Button variant="ghost" size="icon-sm" aria-label="Suppress"
                    ...
                    onClick={() => onSuppress(g.rep, g.hosts.length > 1 ? "global" : "host")}>
